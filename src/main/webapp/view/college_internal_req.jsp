...
<body>
			...
			<div class="min-height-200px">
				<div class="pd-20 bg-white border-radius-4 box-shadow mb-30">
					...
					<c:if test="${not empty internal_req_Role3}">
							<table class="data-table table hover nowrap">
								...
								<tbody id="tablist">
									<c:forEach items="${internal_req_Role3}" var="item">
										<c:if test="${role eq ProjectConstants.hod_Role || role eq ProjectConstants.dept_incharge_Role}">
											<tr>
												<td></td>
												<td>${item[9]}</td>
												<td><span><c:if test="${user_data.user_id eq item[0] }">${self}</c:if>
														<c:if test="${user_data.user_id ne item[0] }">${coordinator}</c:if></span>
												</td>
												<td><span>
													<fmt:formatDate value="${item[10]}" pattern="${ProjectConstants.date_format}" />
													</span></td>
												...
												<td>
													<c:if test="${item[5] eq ProjectConstants.coll_internal_req_approval_status_HOD}">
														<c:if test="${role eq ProjectConstants.hod_Role}">
														...
														<c:if test="${role eq ProjectConstants.dept_incharge_Role}">
															<b><span>Pending for HOD</span></b>
														</c:if>
													</c:if>
													<c:if test="${item[5] eq ProjectConstants.coll_internal_req_approval_status_III}">
														<b><span>Pending at III Cell</span></b>
													</c:if>
													<c:if test="${item[5] eq ProjectConstants.coll_internal_req_approval_status_approved}">
														<b><span class="Acolor">Approved & Sent to Company</span></b>
													</c:if>
													<c:if test="${item[5] eq ProjectConstants.coll_internal_req_approval_status_rejected}">
										...
										<c:if test="${role eq ProjectConstants.iii_Role}">
											<tr>
												...
												<td><span class="dept${item.dept_name}" id="${item.dept_name}">${item.dept_name}
													</span></td>
												<td><span>
													<fmt:formatDate value="${item.creation_date}" 
														pattern="${ProjectConstants.date_format}"/>
													</span></td>
												<td><span id="${item[1]}">${item.company_name}</span></td>
												<td><span class="pointer intrngrp open_pop_up" onclick="studentId(this);" 
													data-toggle="modal" data-target="#all_interns_details" id="${item.for_users}">
														<strong>Group Details</strong>
													</span></td>
												<td><span>${item.expiration_date}</span></td>
												<td>
													<c:if test="${item.approval_status eq ProjectConstants.coll_internal_req_approval_status_III}">
														<button class='btn-success' style="padding: 2px 4px 3px 3px;border-radius: 4px;" onclick="approveReqId('${item.request_id}');">
															Approve</button>
														<button class='btn-primary ${item.request_id}' data-toggle='modal' style="padding: 2px 4px 3px 3px;border-radius: 4px;"
															data-target='#rejectionModal' onclick="rejectReqId('${item.request_id}');">
															Reject</button>
													</c:if>
													<c:if test="${item.approval_status eq ProjectConstants.coll_internal_req_approval_status_approved}">
														<b style="padding: 1px 5px 3px 5px;background: #3eacdec9; color: white;border-radius: 2px;">Request Send to Company</b>
													</c:if>
													<c:if test="${item.approval_status eq ProjectConstants.coll_internal_req_approval_status_rejected}">
															<b><span class="Rcolor">Rejected</span></b>
															<span onclick="rejectionReason(this);" id="${item.request_id}">
																<i class="fa fa-info-circle" aria-hidden="true" title="Rejection Reason"
																	style="color:#0073b7; padding-left:3px;"></i>
													</c:if>
													<c:if test="${item.approval_status eq ProjectConstants.request_cancelled_by_system}">
														<span title="Internship started in some other company.">Cancelled</span>
													</c:if>
												</td>
												<td>${item.request_msg}
													<c:if test="${item.approval_status eq ProjectConstants.coll_internal_req_approval_status_III}">
														<span class="pointer" style="color: green;" 
															onclick="changeRequestMessage('${item.request_id}','${item.request_msg}')">
														<i class="fa fa-pencil-square-o" aria-hidden="true"></i></span>
													</c:if>
												</td>
											</tr>
						...
						<c:if test="${role eq ProjectConstants.dept_incharge_Role || role eq ProjectConstants.hod_Role}">
	...
	<script src="/dist/js/myjs/request_for_internship.js"></script>
